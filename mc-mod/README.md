...
**active abilities** with cooldowns, a **500-level** climb per skill, a **death XP
...
Every skill levels **1 → 500** on its own XP curve, stored as a persisted player
...
| System | What it does |
| --- | --- |
| **Talents** | Each skill has its own 5-talent tree (max rank 5 each). You earn **1 talent point per 20 levels** in that skill — 25 points by level 500, exactly enough to **max every talent** in that skill's tree. Spend them on the Talent screen (Menu ▸ Talent Tree). `/voxelia talent reset` refunds everything. |
...
| **Perk unlocks** | Every level-gated perk announces itself with a slim toast at the top of the screen and a chat line: each skill's **signature ability** (all at level **75** in that skill — Leap at Acrobatics 75, Volley at Archery 75, and so on) plus the four passives — **Haste** (Mining 25), **Telekinesis** (Mining 100), **Last Stand** (Defense 20) and **Well Fed** (Cooking 20). The toast names the perk and tells you how to use it, and reads the real levels from config. |
...
The server is authoritative for everything: XP, talents, abilities and
...
- **`config/voxelia_mmo-common.toml`** — every XP rate and reward coefficient,
  talent rules (`maxRank` — points per level are derived so a maxed skill fills
  its tree exactly), every ability cooldown, and the death penalty
  (`deathXpLossPercent`).
...
└── src/
    ...
    │   ├── skill/        Skill, SkillCurve, PlayerSkills, PlayerTalents, Talent
    │   ├── registry/     VoxeliaAttachments, VoxeliaEntityAttributes
    │   ├── progression/  Progression, SkillEffects, TalentLogic, Abilities
    ...
    │   └── client/       screens (Skills/Talent/Profile/Leaderboard), HUD, sidebar,
